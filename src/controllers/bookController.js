...
const getParticularBooks = async function (req,res){
    ...
    let ParticulatBooks= await bookModel.find({condition}).select({bookName:1,authorName:1, _id:0,tags:1,prices:1,totalPages:1,stockAvailable:1,year:1})
    res.send({msg:ParticulatBooks})
}

const getRandomBooks = async function(req,res){
    let randombooks =await bookModel.find({ $or :[ {totalPages:{ $gt: 200   }}]})
    res.send({msg:randombooks})
}
const getXINRBooks = async function(req,res){
    let INRBooks =await bookModel.find({"prices.indianPrice":{$in:["100INR", "200INR","500INR","600INR"]}})
